    ...
    <section className="w-full">
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
        {projects.map((project) => (
          <Card key={project.title} className="group overflow-hidden border-primary/30 bg-card transition-all hover:border-primary hover:shadow-[0_0_20px_hsl(var(--primary)/0.3)]">
            <CardHeader>
              {project.image && (
            ...
            <CardContent>
              <CardTitle className="mb-2 text-2xl text-primary transition-colors group-hover:text-glow glitch-hover">{project.title}</CardTitle>
              <p className="text-foreground/80">{project.description}</p>
            </CardContent>
            <CardFooter className="flex-col items-start gap-4">
               <div className="flex flex-wrap gap-2">
                {project.tags.map((tag) => (
                  <Badge key={tag} variant="secondary" className="border-primary/20 bg-primary/10 text-primary">
                    {tag}
                  </Badge>
                ))}
              </div>
